...
const UserTable = () => {
  ...
  const [userStates, setUserStates] = useState<{ [key: number]: boolean }>({});
  const [detailUser, setDetailUser] = useState(null);

  const fetchAllUsers = async () => {
    const res = await userServices.getAllUsers(page, limit, search, role);
    return res;
  };
  ...
  const handleFilterRole = (roleValue: string) => {
    setRole(roleValue);
    setPage(1);
  };
  ...
  const handleUpdateStatus = async (userId: number, currentStatus: boolean) => {
    ...
    try {
      const res = await userServices.updateStatusUser(userId, newStatus);
      if (res.status === "Err") {
        toast.error(res.message);
        setUserStates((prev) => ({ ...prev, [userId]: currentStatus }));
        return;
      }
  ...
  const handleDeleteUser = async (userId: number) => {
    try {
      const res = await userServices.deleteUser(userId);
      if (res.status === "Err") {
        toast.error(res.message);
        return;
      }
      ...
      toast.error(e.response?.data?.message || "Lỗi xóa người dùng");
    }
  };
          ...
          <tbody>
            {users?.data?.map((user: any, index: number) => (
              <tr className="border-b border-gray-200" key={user.id}>
                ...
                <th className="px-[24px] py-[16px] text-left font-light">
                  <ToggleSwitch
                    isOn={userStates[user.id] ?? user.is_active}
                    onToggle={() =>
                      handleUpdateStatus(
                        user.id,
                        userStates[user.id] ?? user.is_active
                      )
                    }
                  />
                </th>
