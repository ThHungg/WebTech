...
const CartHeader = ({cartData }: {cartData: any}) => {
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold">Giỏ hàng của tôi</h3>
            </div>
            <h6>{cartData?.cart?.items?.length || 0} sản phẩm</h6>
          </div>
        </div>
        <Link href={"/"} className="border p-2 rounded-2xl hover:border-red-500 hover:text-red-500 transition-all">Tiếp tục mua sắm</Link>
      </div>
    </header>
  );
};
